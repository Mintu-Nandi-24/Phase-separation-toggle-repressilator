% repressilator_phase_sep_sweep_Z.m - Z PHASE SEPARATION ONLY
%
% Modified to use robust oscillation analysis
% Only Z participates in phase separation
% X and Y do NOT participate in phase separation (no dense phases)
...
% Phase separation threshold (only for Z)
...
t0 = 0; tf = 4500;
t_eval = linspace(t0, tf, 2000);
% State vector: [x_dilt; y_dilt; z_dilt; z_dens]
y0 = [30; 38; 31; 0.0];
...
% =============================================================================
fprintf('\n========================================\n');
fprintf('Running parameter sweep for z* from %d to %d (Z phase separation only)\n', min(z_star_values), max(z_star_values));
fprintf('========================================\n\n');
...
for i = 1:numel(z_star_values)
    ...
    % RHS handle with parameters (using Z-only version)
    rhs = @(t,y) repressilator_rhs_Z_only(t, y, params);
...
xlabel('z^*');
ylabel('Amplitude');
title('Amplitude vs z^* (Z Phase Separation Only) - Robust Analysis');
...
xlabel('z^*');
ylabel('Period');
title('Period vs z^* (Z Phase Separation Only) - Robust Analysis');
...
for zs = demo_vals
    ...
    xlabel('Time');
    ylabel('Copy Number');
    title(sprintf('Repressilator time series at z^* = %d (Z Phase Separation Only)', zs));
...
sgtitle('Hopf Bifurcation Diagram with z^* as Tuning Parameter (Z Phase Separation Only)', ...
...
grid on; box on;
xlabel('z^* (Phase Separation Threshold for Z)', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('Copy Number', 'FontSize', 14, 'FontWeight', 'bold');
title('Combined Hopf Bifurcation Diagram (Z Phase Separation Only)', 'FontSize', 16, 'FontWeight', 'bold');
legend({'x_{dilt} (no PS)', '', 'y_{dilt} (no PS)', '', 'z_{dilt} (with PS)', ''}, ...
...
fclose(fid);
fprintf('Exported: x_branch_Z_PS.dat\n');

% Y species
fid = fopen(fullfile(export_dir, 'y_branch_Z_PS.dat'), 'w');
...
fclose(fid);
fprintf('Exported: y_branch_Z_PS.dat\n');

% Z species
fid = fopen(fullfile(export_dir, 'z_branch_Z_PS.dat'), 'w');
...
fclose(fid);
fprintf('Exported: z_branch_Z_PS.dat\n');
...
for zs = demo_vals
    ...
    if isfield(demo_store, key)
        ...
        fclose(fid);
        fprintf('Exported: timeseries_z%d_Z_PS.dat\n', zs);
    end
end
...
function dydt = repressilator_rhs_Z_only(~, y, p)
    % Z PHASE SEPARATION ONLY VERSION: Only Z phase separates
    % X and Y do NOT participate in phase separation (no dense phases)
    % State vector: [x_dilt; y_dilt; z_dilt; z_dens]
    ...
    % Total population for phase-separating species Z
    z_bar = z_dilt + z_dens;
    ...
    % Initialize phase separation rates for Z only
    k_in_z = 0; k_out_z = 0;
    ...
    % Dynamics (X and Y have no phase separation terms)
    ...
    dydt = [dxdilt_dt; dydilt_dt; dzdilt_dt; dzdens_dt];
end
