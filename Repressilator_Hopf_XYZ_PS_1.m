% repressilator_phase_sep_sweep.m - WITH HOPF BIFURCATION DIAGRAM
%
% Modified to use robust oscillation analysis
% All three species (X, Y, Z) participate in phase separation
...
phi_star_x = x_star / volume_factor;
phi_star_y = y_star / volume_factor;
mu_x = -thermal_energy * (log(phi_star_x) - phi_star_x);
mu_y = -thermal_energy * (log(phi_star_y) - phi_star_y);

% Integration settings
t0 = 0; tf = 4500;
t_eval = linspace(t0, tf, 2350); %2250
...
% =============================================================================
fprintf('\n========================================\n');
fprintf('Running parameter sweep for z* from %d to %d (XYZ phase separation)\n', min(z_star_values), max(z_star_values));
fprintf('========================================\n\n');
...
for i = 1:numel(z_star_values)
    ...
    % RHS handle with parameters
    rhs = @(t,y) repressilator_rhs_corrected(t, y, params);
...
xlabel('z^*');
ylabel('Amplitude');
title('Amplitude vs z^* (XYZ Phase Separation) - Robust Analysis');
...
xlabel('z^*');
ylabel('Period');
title('Period vs z^* (XYZ Phase Separation) - Robust Analysis');
...
for zs = demo_vals
    ...
    xlabel('Time');
    ylabel('Copy Number');
    title(sprintf('Repressilator time series at z^*=%d (XYZ Phase Separation)', zs));
    legend('x_{dilt}','y_{dilt}','z_{dilt}','Location','best');
    
    % Add vertical line indicating steady-state start for bifurcation
...
title('Hopf Bifurcation: x_{dilt} Oscillation Envelope', 'FontSize', 14, 'FontWeight', 'bold');
...
title('Hopf Bifurcation: y_{dilt} Oscillation Envelope', 'FontSize', 14, 'FontWeight', 'bold');
first_osc = find(osc_y, 1);
if ~isempty(first_osc) && first_osc > 1
    xline(z_star_values(first_osc), 'b--', 'LineWidth', 2, 'Label', 'Hopf Point');
end

% Z species bifurcation
subplot(3,1,3);
hold on;
for i = 1:length(z_star_values)
    if osc_z(i)
...
title('Hopf Bifurcation: z_{dilt} Oscillation Envelope', 'FontSize', 14, 'FontWeight', 'bold');
first_osc = find(osc_z, 1);
if ~isempty(first_osc) && first_osc > 1
    xline(z_star_values(first_osc), 'b--', 'LineWidth', 2, 'Label', 'Hopf Point');
end

sgtitle('Hopf Bifurcation Diagram with z^* as Tuning Parameter (XYZ Phase Separation)', ...
...
grid on; box on;
xlabel('z^* (Phase Separation Threshold)', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('Copy Number', 'FontSize', 14, 'FontWeight', 'bold');
title('Combined Hopf Bifurcation Diagram (XYZ Phase Separation)', 'FontSize', 16, 'FontWeight', 'bold');
...
if ~exist('exported_data', 'dir')
    mkdir('exported_data');
end

% 1. Export amplitude vs z_star data
fid = fopen('exported_data/amplitude_vs_zstar_XYZ_PS.dat', 'w');
fprintf(fid, 'z_star\tamp_x\tamp_y\tamp_z\n');
for i = 1:length(z_star_values)
    fprintf(fid, '%.1f\t%.6f\t%.6f\t%.6f\n', z_star_values(i), amp_x(i), amp_y(i), amp_z(i));
end
fclose(fid);
fprintf('Exported: amplitude_vs_zstar_XYZ_PS.dat\n');

% 2. Export period vs z_star data
fid = fopen('exported_data/period_vs_zstar_XYZ_PS.dat', 'w');
fprintf(fid, 'z_star\tper_x\tper_y\tper_z\n');
for i = 1:length(z_star_values)
    fprintf(fid, '%.1f\t%.6f\t%.6f\t%.6f\n', z_star_values(i), per_x(i), per_y(i), per_z(i));
end
fclose(fid);
fprintf('Exported: period_vs_zstar_XYZ_PS.dat\n');

% 3. Export bifurcation data (max/min values)
% X species
fid = fopen('exported_data/x_branch_XYZ_PS.dat', 'w');
...
fclose(fid);
fprintf('Exported: x_branch_XYZ_PS.dat\n');

% Y species
fid = fopen('exported_data/y_branch_XYZ_PS.dat', 'w');
...
fclose(fid);
fprintf('Exported: y_branch_XYZ_PS.dat\n');

% Z species
fid = fopen('exported_data/z_branch_XYZ_PS.dat', 'w');
...
fclose(fid);
fprintf('Exported: z_branch_XYZ_PS.dat\n');
...
for zs = demo_vals
    ...
    if isfield(demo_store, key)
        ...
        filename = sprintf('exported_data/timeseries_z%d_XYZ_PS.dat', zs);
        fid = fopen(filename, 'w');
        fprintf(fid, 'time\tx_dilt\ty_dilt\tz_dilt\n');  % Only dilute species for clarity
        ...
        for i = 1:step:length(t)
            fprintf(fid, '%.6f\t%.6f\t%.6f\t%.6f\n', ...
                t(i), y(i,1), y(i,3), y(i,5));
        end
        fclose(fid);
        fprintf('Exported: timeseries_z%d_XYZ_PS.dat\n', zs);
    end
end
...
function dydt = repressilator_rhs_corrected(~, y, p)
    % CORRECTED VERSION: Each species phase separates independently
    % p is a structure containing all parameters
    ...
    % ---- Phase separation in X (INDEPENDENT) ----
    if (x_dilt >= p.x_star || x_dens > 0)
        deltaF_x = free_energy_local(x_dilt + 1, x_bar, p.mu_x, p.thermal_energy, p.volume_factor) ...
                 - free_energy_local(x_dilt, x_bar, p.mu_x, p.thermal_energy, p.volume_factor);
    ...
    % ---- Phase separation in Y (INDEPENDENT) ----
    ...
    % ---- Phase separation in Z (INDEPENDENT) ----
    ...
    % Dynamics
    dxdilt_dt = prod_x - p.b_x * x_dilt - k_in_x + k_out_x;
    dxdens_dt = k_in_x - k_out_x - p.b_x * x_dens;
    ...
    dydt = [dxdilt_dt; dxdens_dt; dydilt_dt; dydens_dt; dzdilt_dt; dzdens_dt];
end
